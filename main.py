...
import numpy as np
import pyotp, datetime, pickle, os
import skimage
import re
import preprocess
import datacrypt
...
class MainApp(Tk):
    ...
    def decrypt_error(self):
    		messagebox.showinfo("Error", "Incorrect sign/password or data been modified (acnhor).")
...
class SignFrame(Frame):
	...
	def paint(self, event):
		if self.x and self.y:
			self.c.create_line(self.x, self.y, event.x, event.y, width=10, fill='black', capstyle=ROUND, smooth=True, splinesteps=36)
		self.x = event.x
		self.y = event.y
...
class AuthFrame(Frame):
	def __init__(self, parent):
		...
		self.counter = -1
		self.counter_totp = pyotp.TOTP("MNXXK3TUMVZGM33SORUW2ZLS")
		self.dict_name = "dict"
		self.dict = {}
	...
	def countdown(self):
		ms = self.counter_totp.interval - datetime.datetime.now().timestamp() % self.counter_totp.interval
		self.counter = int(ms)
